...
function getStatusBadge(s) {
  const info = STATUS_MAP[s] || { label: s || "—", cls: "cancelled" };
...
// ---------------------- جلب الطلبات مع Fallback ----------------------
async function fetchUserOrdersWithFallback(uid) {
  // المحاولة الأولى: مع orderBy
  try {
    const q = query(
...
function renderOrders(orders) {
  ...
  if (orders.length === 0) {
    ...
    e.innerHTML = `
      ...
      <div class="dash-empty-desc">لم تُرسل أي طلبات بعد</div>
    `;
    ordersContent.appendChild(e);
    return;
  }
  ...
  orders.forEach((o) => {
    ...
    item.append(num, title, date, badge);
...
async function init(user) {
  ...
  const orders = await fetchUserOrdersWithFallback(user.uid);
  renderOrders(orders);
